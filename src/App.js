...
const App = () => {
  ...
  console.log(state.columns["column-0"].items.length);
  console.log(state.columns["column-1"].items.length);
  ...
  const onScroll = (element, affectedElement) => {
    affectedElement.scrollTop(element.scrollTop());
  };
  ...
  useEffect(() => {
    ...
    firstColumn.on({ scroll: () => onScroll(firstColumn, secondColumn) });
    secondColumn.on({ scroll: () => onScroll(secondColumn, firstColumn) });
  }, []);
